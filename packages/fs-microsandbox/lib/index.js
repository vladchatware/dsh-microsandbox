...
import { MicrosandboxError, quoteShellArg } from "@deepseek-ai/dsh-microsandbox";
...
function decodeText(bytes, displayPath, binarySampleBytes) {
	if (bytes.subarray(0, binarySampleBytes).includes(0)) throw new FsError(`cannot read "${displayPath}": binary file`, "FS_NOT_TEXT");
	try {
...
function decodeCanonicalPath(encoded) {
	if (encoded.length === 0 || !BASE64.test(encoded)) throw new Error("fs-microsandbox: canonical path transport returned invalid base64");
	const framed = Buffer.from(encoded, "base64");
	if (framed.toString("base64") !== encoded || framed.length < 2 || framed.at(-1) !== 0 || framed.subarray(0, -1).includes(0)) throw new Error("fs-microsandbox: canonical path transport returned invalid NUL framing");
	let path;
	try {
...
function mapError(error, operation, displayPath, signal) {
	...
	if (signal?.aborted === true || error instanceof DOMException && error.name === "AbortError") return new FsError(`${operation} aborted`, "FS_ABORTED", { cause: error });
	if (isNotFound(error)) return new FsError(`cannot ${operation} "${displayPath}": not found`, "FS_NOT_FOUND", { cause: error });
	if (isPermission(error)) return new FsError(`cannot ${operation} "${displayPath}": permission denied`, "FS_PERMISSION_DENIED", { cause: error });
...
function entryVersion(path, entry) {
...
var MicrosandboxFileSystem = class extends FileSystem {
	...
	async lstat(path, opts, signal) {
		...
		const displayPath = posix.resolve(opts?.cwd ?? this.ctx.microsandbox.cwd, path);
		try {
			const result = await (await this.ctx.microsandbox.getSandbox()).exec("bash", ["-c", `LC_ALL=C stat -c '%F|%s|%Y' -- ${quoteShellArg(displayPath)} 2>/dev/null || echo MISSING`]);
			assertNotAborted(signal, "lstat");
			const line = result.stdout().trim();
			if (line === "MISSING") return void 0;
			const [kind, sizeText, mtimeText] = line.split("|");
			const type = kind === "symbolic link" ? "symlink" : kind === "regular file" || kind === "regular empty file" ? "file" : kind === "directory" ? "directory" : "other";
			const facts = JSON.stringify([
				displayPath,
				kind,
				sizeText,
	...
	async readText(target, signal) {
		...
		try {
			const bytes = await (await this.ctx.microsandbox.getSandbox()).fs().read(String(target.targetKey));
	...
	async readBytes(target, signal, maxBytes) {
		...
		try {
			const stream = await (await this.ctx.microsandbox.getSandbox()).fs().readStream(String(target.targetKey));
			const chunks = [];
			let bytes = 0;
			let completed = false;
			try {
	...
	async streamText(target, signal) {
		...
		return { async *[Symbol.asyncIterator]() {
			let stream;
			try {
				stream = await (await owner.getSandbox()).fs().readStream(targetKey);
			} catch (error) {
	...
	async listDir(target, signal) {
		...
		try {
			...
			for (const entry of listed) {
				...
				const canonical = entry.kind === "symlink" ? await this.canonicalPath(sandbox, entry.path, signal) : entry.path;
				const resolved = entry.kind === "symlink" ? await this.probe(canonical, displayPath, signal) : void 0;
				entries.push({
					name,
					type: resolved === void 0 ? entry.kind === "file" ? "file" : entry.kind === "directory" ? "directory" : "other" : entryType(resolved),
					target: {
						targetKey: FsTargetKey(canonical),
						displayPath
					},
					...resolved !== void 0 ? { version: entryVersion(canonical, resolved) } : {},
	...
	async writeText(target, content, expected, signal) {
		return this.withLock(String(target.targetKey), async () => {
			...
			if (existing !== void 0 && entryType(existing) !== "file") throw new FsError(`cannot write "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
	...
	async editText(target, edit, expected, signal) {
		return this.withLock(String(target.targetKey), async () => {
			...
			if (entryType(existing) !== "file") throw new FsError(`cannot edit "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
			if (expected !== void 0 && entryVersion(String(target.targetKey), existing) !== expected.version) throw new FsError(`cannot edit "${target.displayPath}": file changed since it was read`, "FS_STALE_VERSION");
	...
	async canonicalPath(sandbox, path, signal) {
		...
		try {
			return decodeCanonicalPath((await sandbox.exec("bash", ["-c", `set -o pipefail; realpath -mz -- ${quoteShellArg(path)} | base64 -w0`])).stdout());
		} catch (error) {
	...
	async probe(path, displayPath, signal) {
		...
		try {
			const sandbox = await this.ctx.microsandbox.getSandbox();
			if (!await sandbox.fs().exists(path)) return void 0;
			assertNotAborted(signal, "stat");
	...
	checkWriteIntent(existing, expected, target) {
		...
		if (expected?.kind === "replaceIfVersion") {
			if (existing === void 0 || entryVersion(String(target.targetKey), existing) !== expected.version) throw new FsError(`cannot write "${target.displayPath}": file changed since it was read`, "FS_STALE_VERSION");
		}
	}
	...
	async writeAtomic(target, content, existing, createIfAbsent, signal) {
		...
		try {
			...
			if (createIfAbsent) {
				...
				const publication = await sandbox.exec("bash", ["-c", `if ln -T -- ${quoteShellArg(temporary)} ${targetArg}; then printf created; elif test -e ${targetArg} || test -L ${targetArg}; then printf exists; else exit 1; fi`]);
				if (publication.stdout() === "exists") throw new FsError(`cannot overwrite existing "${target.displayPath}" without reading it first`, "FS_NOT_OBSERVED");
				if (publication.stdout() !== "created") throw new Error("guarded create returned an invalid publication result");
			} else await sandbox.fs().rename(temporary, targetPath);
			try {
			...
			if (committed === void 0) throw new Error(`write committed but target vanished: ${targetPath}`);
			return entryVersion(targetPath, committed);
		} catch (error) {
			if (stagingDirectoryCreated) try {
				await sandbox.fs().removeDir(stagingDirectory);
...
//#endregion
export { MicrosandboxFileSystem, MicrosandboxFileSystem as default };
