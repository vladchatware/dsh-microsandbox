...
import { quoteShellArg, MicrosandboxError } from '@deepseek-ai/dsh-microsandbox';
...
function decodeCanonicalPath(encoded) {
    ...
    if (!posix.isAbsolute(path))
        throw new Error('fs-microsandbox: canonical path is not absolute');
    return path;
}
...
export class MicrosandboxFileSystem extends FileSystem {
    ...
    fileUrl(target) {
        ...
        if (!posix.isAbsolute(path))
            throw new Error(`fs-microsandbox: expected an absolute process path: ${JSON.stringify(path)}`);
