...
*/
const name = "tool-microsandbox";
const inject = ["tools", "microsandbox"];
/** Runtime configuration schema for the vm_bash tool plugin. */
const Config = Schema.object({
	maxOutputBytes: Schema.number().default(65536),
	timeoutMsCap: Schema.number().default(6e4)
});
/**
...
var TailSink = class {
	...
	push(data) {
		...
		const text = Buffer$1.from(data).toString("utf8");
		const textBytes = Buffer$1.byteLength(text);
		...
		while (this.bytes > this.cap && this.chunks.length > 1) {
			...
			this.bytes -= Buffer$1.byteLength(head);
		}
		if (this.bytes > this.cap) {
			const kept = this.chunks.join("").slice(-this.cap);
			this.chunks = [kept];
			this.bytes = Buffer$1.byteLength(kept);
		}
	}
	text() {
...
function renderResult(value) {
	...
	if (value.timedOut) lines.push(`[timed out after ${value.timeoutMs}ms; process group killed]`);
	lines.push(`[exit code: ${value.exitCode}]`);
	return lines.join("\n");
}
async function runVmBash(ctx, args, config) {
	if (typeof args.command !== "string" || args.command.trim().length === 0) throw new Error("vm_bash: command must be a non-empty string");
	const workdir = typeof args.workdir === "string" && args.workdir.length > 0 ? args.workdir : ctx.microsandbox.cwd;
	if (!posix.isAbsolute(workdir)) throw new Error(`vm_bash: workdir must be an absolute VM path: ${JSON.stringify(workdir)}`);
	...
	const execHandle = await sandbox.execStreamWith("bash", (builder) => builder.args(["-c", args.command]).cwd(workdir));
			...
			try {
				for (;;) {
					...
					if (event.kind === "started") {
						...
						timer = setTimeout(() => {
							timedOut = true;
							guestKillGroup(sandbox, pgid).catch(() => {});
						}, timeoutMs);
					} else if (event.kind === "stdout") stdout.push(event.data);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			command: {
				type: "string",
				required: true,
				description: "The bash command to run in the microVM."
			},
			description: {
				type: "string",
				required: true,
			...
			workdir: {
				type: "string",
				description: "Working directory inside the VM (default: the VM's /workspace)."
			},
			timeoutMs: {
				type: "number",
				description: `Timeout in milliseconds; the process group is killed on expiry. Capped at ${runtimeConfig.timeoutMsCap}.`
			}
		},
		output: {
			schema: {
				...
				properties: {
					exitCode: {
						type: "integer",
						required: true,
						description: "Exit code; -1 when killed by signal or timeout."
					},
					stdout: {
						type: "string",
						required: true
